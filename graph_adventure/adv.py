...
# TRAVERSAL TEST
visited = {}
moving_direction = {"n":"s","s":"n","e":"w","w":"e"}

# print(f'player current room: {player.currentRoom.id}')

# print(f'player current room exits: {player.currentRoom.getExits()}')

room_exit = player.currentRoom.getExits()
#

room_id = player.currentRoom.id
room_id_exit={}
room_id_exit[room_id] = room_exit
# print(f'initial room id exit dict: {room_id_exit}')

prev = {}

def bfs(starting_vertex):
    visited_room = set()
    # print(f'bfs room starting: {starting_vertex}')
    queue = Queue()
    ...
    while queue.size() > 0:
        ...
        visited_room.add(last_room)
        
        for room_direction in visited[last_room]:
            
            if visited[last_room][room_direction] == "?":
                # print(f'<- bfs last room id: {player.currentRoom.id}')
                return path
            else:

                if visited[last_room][room_direction] not in visited_room:
                    new_path = list(path)
                    new_path.append(visited[last_room][room_direction])
...
while len(visited) <= len(roomGraph):
    #add to visited only if the room does not exists
    if room_id not in visited:
        temp={}
        for i in room_id_exit[room_id]:
            temp[i] = "?"
        visited[room_id] = temp

        # print(f'newly added visited room: {visited}')

    else:
        # update the current room direction from the room it came from.
        if room_id in prev:
            extract_dict = prev[room_id]
            # print(f'extract dict pair {extract_dict}')

            for j in visited[room_id]:
                # print(f'directions in visited: {j}')
                if j in extract_dict:
                    visited[room_id][j] = extract_dict[j]

        # print(f'inside while: if: {visited}\n')

        #picks a direction which is unexplored ("?") in the current room and which is not in visited
        possible_room_direction = []
        for direction in visited[room_id]:
            if visited[room_id][direction] == "?" and player.currentRoom.getRoomInDirection(direction).id not in visited:
                possible_room_direction.append(direction)

        # print(f'dft: possible directions: {possible_room_direction}')

        #player travels to an unexplored room and updates the direction from the room it came from.
        # player also stores the room and the exits of the unexplored room.
        if possible_room_direction:
            # print(f'first picked room direction: {possible_room_direction[0]}')
            player.travel(possible_room_direction[0])
            traversalPath.append(possible_room_direction[0])
            # print(f'traversal path: {traversalPath}')
            visited[room_id][possible_room_direction[0]] = player.currentRoom.id
            prev[player.currentRoom.id] = {moving_direction[room_exit[0]]:room_id}
            # print(f'player current room after travelling: {player.currentRoom.id}')
            room_id = player.currentRoom.id
            room_exit = player.currentRoom.getExits()
            room_id_exit[player.currentRoom.id]=player.currentRoom.getExits()

            #Doing a check if the player has visited all rooms
            if len(visited) == len(roomGraph):
                # print(f'before else: dft length check: traversalPath: {traversalPath}')
                break
        else:
            # if all the rooms are explored and has no "?" marks, doing a bfs to find an unexplored room
            # Or even if the player has hit a deadend, bfs is used to come reverse and helps to find a direction which is still unexplored
            path_unexplored = bfs(player.currentRoom.id)
            
            # print(f'total result from bfs: {path_unexplored}')
            
            #bfs give the room id's
            # converting the room id's to directions
            # player travels to all those rooms
            # the converted directions are added to the traversel path
            if path_unexplored is not None and len(path_unexplored) > 0:
                for i in range(len(path_unexplored)-1):
                    
                    for direction in visited[path_unexplored[i]]:
                        # print(f'dft: direction: {direction} ')
                        # print(f'i: {visited[path_unexplored[i]][direction]}')
                        if visited[path_unexplored[i]][direction] == path_unexplored[i+1]:
                            player.travel(direction)
                            # print(f'dft: inside for: current room: {player.currentRoom.id} ')
                            traversalPath.append(direction)

                # print(f'player current room after append the path from bfs: {player.currentRoom.id}')
                # print(f'traversal path after bfs: {traversalPath}')

                bfs_last_room = player.currentRoom.id

                # After room id's are returned from bfs, the last room the player travelled to may or may not be in visited
                # So adding a new room to the visited list, with its default exits as "?"

                if bfs_last_room not in visited:
                    visited[player.currentRoom.id] = {}
                    for exits in player.currentRoom.getExits():
                        visited[bfs_last_room][exits] = "?"
                    # print(f'inside dft: new room added: {visited}')


                # if the last room returned from bfs is in visited, Doing a check if there is any unexplored direction
                # the player travels to the unexplored direction
                # if the room, the player travelled to is not in visited
                # the room is added to visited with its default exits as "?"
                # the direction he used to go to the unexplored room is added to the traversal Path
                for room_direction in visited[bfs_last_room]:
                    # print(f'inside dft last room for')
                    if visited[bfs_last_room][room_direction] == "?":
                        # print(f'inside dft: current room player at: {player.currentRoom.id}')
                        # print(f'inside dft: visited room : {visited}')
                        # print(f'inside dft:if: player : last room -> {bfs_last_room}, room direction -> {room_direction}')
                        player.travel(room_direction)
                        # print(f'inside dft: after player travels : {player.currentRoom.id}')
                        traversalPath.append(room_direction)
                        # print(f'traversal path after returm from bfs: {traversalPath}')
                        visited[bfs_last_room][room_direction] = player.currentRoom.id
                        if player.currentRoom.id not in visited:
                            visited[player.currentRoom.id] = {}
                            for exits in player.currentRoom.getExits():
                                visited[player.currentRoom.id][exits] = "?"
                        visited[player.currentRoom.id][moving_direction[room_direction]] = bfs_last_room 
                        # print(f'inside dft: after update: visited room : {visited}')
                        # print(f'inside dft: after update: current room: {player.currentRoom.id}')
                        # print(f'inside dft: moving  direction {moving_direction[room_direction]}')
                        break
